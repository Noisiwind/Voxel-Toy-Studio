...
function createDetailedEagle(): Voxel[] {
  ...
  for (let y = 10; y <= 11; y++) {
    voxels.push({ x: 0, y, z: 2, c: beakColor });
    voxels.push({ x: 0, y, z: 3, c: beakColor });
  }
  ...
  for (let x = -8; x <= -2; x++) {
    for (let y = 5; y <= 7; y++) {
      for (let z = -2; z <= 1; z++) {
        ...
        voxels.push({ x, y, z, c: wingColors[colorIndex] });
      }
    }
  }
  ...
  for (let y = 3; y <= 5; y++) {
    for (let z = -5; z <= -2; z++) {
      for (let x = -2; x <= 2; x++) {
        ...
        voxels.push({ x, y, z, c: tailColors[colorIndex] });
      }
    }
  }
  ...
  // 左爪
  voxels.push({ x: -1, y: 3, z: 0, c: clawColor });
  voxels.push({ x: -1, y: 2, z: 0, c: clawColor });
  voxels.push({ x: -1, y: 2, z: 1, c: clawColor });
  voxels.push({ x: -2, y: 2, z: 1, c: clawColor });
  ...
  for (let y = 5; y <= 7; y++) {
    voxels.push({ x: -2, y, z: 0, c: detailColor });
    voxels.push({ x: 2, y, z: 0, c: detailColor });
  }
